...
st.title("SwiftGrasp")
st.write(
    "A web app about a company's performance and people's \
    opinion on it"
)

st.text("\n")
st.write(
    "Disclaimer: The Content is for informational purposes \
...
st.markdown(
    "_Notes: If you have inputs on both, the text input \
...
if not (ct.has_stock or ct.has_statement):
    st.markdown(
        f"_This ticker {ticker} doesn't have any data.\
...
if ct.has_statement:
    ...
    change_dt_list = None
    st.markdown(
        "_The ticker you chose doesn't have financial \
...
if ct.has_stock:
    ...
    df_stock_fill = (
        df_stock.drop_duplicates(subset=date_col, keep="last")
        .set_index(date_col)
        .sort_index()
    )
    # fmt: off
...
st.markdown(
    "For bugs or data issues, please email \
